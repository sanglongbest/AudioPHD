...
settings.NFFT = settings.frameSize; %number of FFT
settings.voiceFre = [1,settings.NFFT];


%Default: [@(x) 1./(abs(x)+(abs(x)<5e-3)),1,2048,1048,800]

%ly [stft, f, t] = stft(a(1,:), settings.frameSize, settings.dx, settings.frameSize*2, settings.sr);


%[TDOA,GPHATi,E] = TDOAmeasurment(a(1,:), a(2,:), settings)
[scores,TDOA] = gccscores(a,settings);
tau = TDOA(:,:,1);

p_1 = 2.4050e-06
p_3 = 1.807e-02
%% Save data:
if ~exist([fileNameBase 'tau.mat'],'file')
    save([fileNameBase 'data'],'settings','tau')
end


% settings.nbrOfFrames = size(scores);
% 
% %Plot:
